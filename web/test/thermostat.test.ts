...
describe("thermostat controller", () => {
  it("shows rapid draft changes immediately but sends one final setpoint", async () => {
    ...
    const controller = createThermostatController({ setSetpoint, setMode: vi.fn() }, state, 650);
  ...
  it("restores the confirmed setpoint after a failed command", async () => {
    vi.useFakeTimers();
    const controller = createThermostatController({ setSetpoint: vi.fn().mockRejectedValue(new Error("offline")), setMode: vi.fn() }, state, 10);
    controller.adjust(1);
